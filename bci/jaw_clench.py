"""
jaw_clench_detector.py  –  Tetris edition
==========================================
Detects single jaw clenches (rotate piece) from a live EEG/EMG channel.
Compared with the Flappy Bird version, the main changes are:

  * refractory raised to 0.50 s  – avoids double-rotations on one clench
  * merge_tol raised to 0.10 s   – cleans up edge artefacts at chunk boundaries
  * baseline_size still 5 s of samples
  * Public API is identical: detect(signal, timestamps, thresh_min)
    returns (peaks, new_clench_times, info_dict)

No other files need to import this directly; tetris_task.py wraps it in a
thin helper that converts clench events → rotate commands.
"""
